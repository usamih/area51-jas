 ...
 * 
 * Reads all records in VEHICLES in ISN order 10 times using L1 commands.
 * 
 * @author usadva
 */
...
public class AdabasGenerateCQTest2 extends JUnitCommon {
	...
	public void generateCQTest() throws Exception {
		...
		for (int i=0; i<10; i++) {
			...
			acbx.setAcbFNR(1);
			acbx.setAcbCMD("OP");
			acbx.setAcbADD1("JASCQTS2");

			AdabasBufferX opFB = new AdabasBufferX(".", AdabasBufferX.FB);
			adcx.setFB(opFB);
			AdabasBufferX opRB = new AdabasBufferX("ACC=11.", AdabasBufferX.RB);
			adcx.setRB(opRB);
			adcx.callAdabas();
			...
			AdabasBufferX L1FB = new AdabasBufferX("AA.", AdabasBufferX.FB);
			adcx.setFB(L1FB);
